...
\begin{comment}
Let's delve into the math. For simplicity, we first consider a single monitored channel \(\hat c\) with rate \(\gamma\).
At each infinitesimal step, the joint state before the measurement is still taken in the factorized form
\begin{equation}
	\hat R(t)=\rho^{(c)}(t)\otimes \dyad{0}{0}_t,
	\label{eq:oqs_factorized_conditional_state}
\end{equation}
and after the infinitesimal interaction,
\begin{equation}
	\hat R(t+dt)
	=
	\hat U(t+dt,t)\,
...
We begin with spontaneous emission. In the convention adopted throughout this thesis, \(\hat \sigma_z\ket{0}=+\ket{0}\) and \(\hat \sigma_z\ket{1}=-\ket{1}\), so that \(\ket{0}\) is the ground state and \(\ket{1}\) is the excited state. The lowering and raising operators are therefore
\begin{equation}
	\hat \sigma_- = \dyad{0}{1},
	\qquad
	\hat \sigma_+ = \dyad{1}{0}.
