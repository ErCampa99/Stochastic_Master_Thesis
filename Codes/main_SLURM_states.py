...
def runs_states_to_array(runs_states) -> np.ndarray:
    ...
    for i, run in enumerate(runs_states):
        if len(run) != nsteps:
            raise ValueError(f"Inconsistent number of time steps in trajectory {i}: {len(run)} != {nsteps}")
        for j, state in enumerate(run):
            out[i, j] = state.full()
    return out
...
def run_homodyne(
    ...
    return runs_states_to_array(sol.runs_states)
...
def main() -> None:
    ...
    #Organize the results into a dictionary mapping eta values to states arrays, and prepare the output directory and metadata for saving the results.
    states_map = {key: states for key, states in results}
    ...
    for eta_1, eta_2 in etas:
        ...
        state_npz_files.append(eta_npz_path.name)
    ...
    print(f"Done. States saved in: {out_dir}")
