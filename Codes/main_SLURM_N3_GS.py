"""
Homodyne single dephasing simulation for N=3 atoms, with Jz measurement as plus channel.
...
PlusPlusPlus = tensor(plus_state, plus_state, plus_state)
ee = tensor(exc, exc, exc)

# Function to create a coherent spin state (CSS) for N=3 atoms based on the given angles theta and phi.
def css_2(theta: float, phi: float):
    single_qubit = (
        np.sin(theta / 2.0) * basis(2, 0)
        + np.exp(1j * phi) * np.cos(theta / 2.0) * basis(2, 1)
    )
    return tensor(single_qubit, single_qubit, single_qubit)


# Collective spin operators for N=3
I_2 = qeye(2)
J_x = 0.5 * (
    tensor(sigmax(), I_2, I_2) + tensor(I_2, sigmax(), I_2) + tensor(I_2, I_2, sigmax())
)
J_y = 0.5 * (
    tensor(sigmay(), I_2, I_2) + tensor(I_2, sigmay(), I_2) + tensor(I_2, I_2, sigmay())
)
J_z = 0.5 * (
    tensor(sigmaz(), I_2, I_2) + tensor(I_2, sigmaz(), I_2) + tensor(I_2, I_2, sigmaz())
)
...
# Homodyne channels for N=3 with Gram-Schmidt unitary transformation.
U_GS = np.array(
    [
...
#To construct the collective measurement operators C1, C2, and C3 for N=3 atoms based on the local Z operators (Z_1, Z_2, Z_3) and the Gram-Schmidt unitary transformation (U_GS).
Sz_1 = tensor(sigmaz(), I_2, I_2)
Sz_2 = tensor(I_2, sigmaz(), I_2)
Sz_3 = tensor(I_2, I_2, sigmaz())
Z_local = [Sz_1, Sz_2, Sz_3]

#The collective measurement operators C1, C2, and C3 are obtained by applying the Gram-Schmidt unitary transformation to the local Z operators.
C1 = U_GS[0, 0] * Z_local[0] + U_GS[0, 1] * Z_local[1] + U_GS[0, 2] * Z_local[2]
C2 = U_GS[1, 0] * Z_local[0] + U_GS[1, 1] * Z_local[1] + U_GS[1, 2] * Z_local[2]
C3 = U_GS[2, 0] * Z_local[0] + U_GS[2, 1] * Z_local[1] + U_GS[2, 2] * Z_local[2]

#To create the collapsing operators for N=3 based on the measurement efficiency (eta) and the phases (phi_1, phi_2, phi_3) of the homodyne detection.
def collapsing_operators(gamma: float, phi_1: float, phi_2: float, phi_3: float, eta_1: float, eta_23: float):
    return [
        np.sqrt(gamma*eta_1) * np.exp(1j * phi_1) * C1,
        np.sqrt(gamma*eta_23) * np.exp(1j * phi_2) * C2,
        np.sqrt(gamma*eta_23) * np.exp(1j * phi_3) * C3,
    ]
...
def eta_triplet_key(eta_1: float, eta_23: float) -> str:
    return f"{eta_1:g}_{eta_23:g}"

# Utility function to parse the list of etas from a comma-separated string.
def parse_etas(raw_1: str, raw_23: str) -> list[tuple[float, float]]:
    ...
    #Parse the comma-separated strings for etas_1 and etas_23, convert them to lists of floats, and validate the values.#
    etas_1 = [float(x.strip()) for x in raw_1.split(",") if x.strip()]
    etas_23 = [float(x.strip()) for x in raw_23.split(",") if x.strip()]
    if not etas_1 or not etas_23:
        raise ValueError("Eta list is empty. Use --etas_1 and --etas_23 with comma-separated values.")

    if len(etas_1) == 1 and len(etas_23) > 1:
        etas_1 = etas_1 * len(etas_23)
    elif len(etas_23) == 1 and len(etas_1) > 1:
        etas_23 = etas_23 * len(etas_1)
    elif len(etas_1) != len(etas_23):
        raise ValueError(
            f"Length mismatch: len(etas_1)={len(etas_1)} and len(etas_23)={len(etas_23)}. "
            "Use equal lengths, or one list with a single value to broadcast."
        )

    for eta_1, eta_23 in zip(etas_1, etas_23):
        if eta_1 < 0.0 or eta_1 > 1.0 or eta_23 < 0.0 or eta_23 > 1.0:
            raise ValueError(f"Invalid etas=({eta_1}, {eta_23}). Each eta must be in [0, 1].")
    return list(zip(etas_1, etas_23))
...
def build_initial_state(kind: str, theta: float, phi_state: float):
    if kind == "PlusPlusPlus":
        return PlusPlusPlus, np.pi / 2.0
    if kind == "ee":
        return ee, np.pi
    if kind == "css":
...
def run_homodyne(
    ...
    sol = smesolve(
        ...
        rho0,
        times,
        c_ops=collapsing_operators(gamma, phi1, phi2, phi3, 1.0 - eta_1, 1.0 - eta_23),
        sc_ops=collapsing_operators(gamma, phi1, phi2, phi3, eta_1, eta_23),
        heterodyne=False,
        e_ops=e_ops,
        ntraj=ntraj,
        options=options,
...
def simulate_single_eta(
    ...
    key = eta_triplet_key(float(eta_1), float(eta_23))
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="N=3 homodyne simulation (Jz, Gram-Schmidt unitary)")

    #The user can choose the initial state from predefined options (PlusPlusPlus, ee, css) and specify parameters for the CSS state if chosen. 
    ...
    parser.add_argument("--state", choices=["PlusPlusPlus", "ee", "css"], default="PlusPlusPlus")
    ...
    parser.add_argument("--phi3", type=parse_angle_arg, default=0.0, help="Angle (e.g. 0, pi/2, np.pi/2)")
    ...
    parser.add_argument("--etas_23", type=str, default="1,0.8,0.5,0.3,0")
    ...
    parser.add_argument(
        "--out-root",
        type=str,
        default=r"./Graphs/Jz_3_Homodyne_N3_GS",
    )
    parser.add_argument(
        "--no-theory",
        action="store_true",
...
def main() -> None:
    ...
    etas_all = parse_etas(args.etas_1, args.etas_23)
    ...
    meta = {
        ...
        "etas_23": [float(eta_23) for _, eta_23 in etas_all],
        "eta_index": eta_index,
    ...
    for col in columns:
        ...
        for eta_1, eta_23 in etas:
            ...
            line, = plt.plot(
                x,
                y,
                label=rf"$\eta_1 = {eta_1:g},\ \eta_{{23}} = {eta_23:g}$",
            )
